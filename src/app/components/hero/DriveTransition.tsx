...
import { BORDERS, SCENES } from './heroScenes'

/**
 * Scene-seam overlay, styled as a *Persona 5* graphic beat: as the drive crosses
 * a border the screen slams into bold angular accent panels + halftone + motion
 * streaks, holds on a near-black accent field (which hides the still swap),
 * then the panels slash off the other side to reveal the next scene while its big
 * italic all-caps location title lingers.
 *
 * Purely a DOM overlay driven by its own rAF. It reads a *time-damped* copy of
 * `progressRef` so a fast scroll flick can't snap the whole beat past in a
 * couple of frames — that was the "transitions are too fast" bug. Layers,
 * back-to-front:
 *   field   — near-black accent wash, peaks at the swap to hide it
 *   panels  — 3 skewed accent/ink/light bands that slide through the seam
 ...
 */
// half-window (scroll progress) around each seam. Must stay < half a scene band
...
// One card per scene, derived from SCENES (so the card always names the scene
// you enter). Indexed by scene 0..N-1; crossing border i drives INTO scene i+1.
const LOCS: Loc[] = SCENES.map((s) => ({ name: s.card.name, sub: s.card.sub, rgb: hexToRgb(s.accent) }))
...
export default function DriveTransition({ progressRef }: { progressRef: RefObject<number> }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // rebuild accent-tinted gradients only when the incoming scene changes
  ...
  return (
    <div
      ...
      aria-hidden="true"
    >
      {/* near-black accent field — peaks at the swap to hide the still cut */}
